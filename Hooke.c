...
/*** FILE OBTAINED FROM http://www.netlib.org/opt/hooke.c ***/
...
#define      VARS		(3)	    /* max # of variables	     */
...
/* BEGIN: ADDED BY Miguel Navascues */
double f(x, n)
	   double	   x[3];
	   int		   n;
{
        ...
        if (option==1) value = p_sample_no_H(x);
        if (option==2) value = p_sample(x);

        funevals++;

        return value;
}
/* END: ADDED BY Miguel Navascues */
	   ...
	   int		   nvars;
{
	   ...
	   for (i = 0; i < nvars; i++) {
		   z[i] = point[i] + delta[i];
		   ftmp = f(z, nvars);//p_sample_no_H(z);//
		   if (ftmp < minf)
			   minf = ftmp;
		   else {
			   delta[i] = 0.0 - delta[i];
	   ...
	   double	   rho, epsilon;
{
	   ...
	   steplength = rho;
	   iters = 0;
	   fbefore = f(newx, nvars);//p_sample_no_H(newx);//
	   newf = fbefore;
	   while ((iters < itermax) && (steplength > epsilon)) {
		   ...
		   while ((newf < fbefore) && (keep == 1)) {
				...
				if (counter > 10000){
					printf("\n TOO MANY ITERATIONS\n");
					fprintf(flog,"Too many iterations in optimization algorithm, maximum not reached\n");
					problem=1;
					endpt[0]=-9;
					endpt[1]=-9;
					endpt[2]=-9;
